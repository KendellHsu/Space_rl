...
background_img = pygame.image.load(os.path.join(BASE_PATH, "img", "background.png")).convert()
...
class Game:
    ...
    def check_for_collisions(self):
        ...
        if self.player.sprite.lives == 0:
            die_sound.play()
            self.draw_text(screen, "Game Over", 42, WIDTH/2, HEIGHT/2-50)
            self.running = False
    ...
    def draw(self, screen):
        ...
        self.draw_text(screen, str(self.score), 18, WIDTH/2, 10)
        self.draw_health(screen, self.player.sprite.health, 5, 15)
        self.draw_lives(screen, self.player.sprite.lives, player_mini_img, WIDTH - 100, 15)
